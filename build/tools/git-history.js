...
export function registerGitHistoryTools(server, deps) {
    const { gitHistoryIndexer } = deps;
    // index_git_history
    server.registerTool("index_git_history", {
        title: "Index Git History",
        description: "Index a repository's git commit history for semantic search. Extracts commit messages, metadata, and optionally diffs to enable finding relevant past commits. Useful for finding similar fixes, understanding change patterns, or learning from past work.",
        inputSchema: schemas.IndexGitHistorySchema,
    }, withToolLogging("index_git_history", async ({ path, forceReindex, sinceDate, maxCommits }, extra) => {
        log.info({ tool: "index_git_history", path, forceReindex }, "Tool called");
        const progressToken = extra._meta?.progressToken;
        const stats = await gitHistoryIndexer.indexHistory(path, { forceReindex, sinceDate, maxCommits }, (progress) => {
    ...
    // search_git_history
    server.registerTool("search_git_history", {
        title: "Search Git History",
        description: "Search indexed git history using natural language queries. Returns semantically relevant commits with metadata. Useful for finding past fixes, similar changes, or understanding how problems were solved before.",
        inputSchema: schemas.SearchGitHistorySchema,
    }, withToolLogging("search_git_history", async ({ path, query, limit, commitTypes, authors, dateFrom, dateTo, }) => {
        log.info({ tool: "search_git_history", path, query: query.substring(0, 80) }, "Tool called");
    ...
    // index_new_commits
    server.registerTool("index_new_commits", {
        title: "Index New Commits",
        description: "Incrementally index only new commits since the last indexing. Much faster than full re-indexing when keeping the index up to date with recent changes.",
    ...
    server.registerTool("get_git_index_status", {
    ...
    // clear_git_index
    server.registerTool("clear_git_index", {
        title: "Clear Git Index",
        description: "Delete all indexed git history data for a repository. This is irreversible and will remove the entire git history index.",
