...
const mockLog = logger.child({ component: "tools" });
describe("withToolLogging", () => {
    ...
    it("should log completion with durationMs on success", async () => {
        ...
        expect(mockLog.info).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('should log warn for search tool with "No results found" response', async () => {
        ...
        expect(mockLog.warn).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it("should log warn for search tool with empty JSON array response", async () => {
        ...
        await wrapped({ collection: "test", query: "test" });
        expect(mockLog.warn).toHaveBeenCalledWith(expect.objectContaining({ tool: "hybrid_search" }), "Tool completed with no results");
    });
    it("should log warn for search tool with Found 0 result(s) response", async () => {
        ...
        expect(mockLog.warn).toHaveBeenCalledWith(expect.objectContaining({ tool: "search_code" }), "Tool completed with no results");
    });
    it("should NOT log warn for non-search tool with empty-looking response", async () => {
        ...
        expect(mockLog.info).toHaveBeenCalledWith(expect.objectContaining({ tool: "get_index_status" }), "Tool completed");
    ...
    it("should log info for search tool with actual results", async () => {
        ...
        const wrapped = withToolLogging("search_code", handler);
        await wrapped({ path: "/test", query: "test" });
        expect(mockLog.info).toHaveBeenCalledWith(expect.objectContaining({ tool: "search_code" }), "Tool completed");
        expect(mockLog.warn).not.toHaveBeenCalled();
    });
    ...
    it("should handle empty content array", async () => {
        ...
        await wrapped({ path: "/test", query: "test" });
        expect(mockLog.warn).toHaveBeenCalledWith(expect.objectContaining({ tool: "search_git_history" }), "Tool completed with no results");
    });
});
