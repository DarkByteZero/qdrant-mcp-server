...
export class CodeIndexer {
    ...
    embeddings;
    config;
    log = logger.child({ component: "code-indexer" });
    ...
    async getCollectionName(path) {
        ...
        try {
            ...
            const { stdout: gitRootResult } = await execFileAsync("git", ["rev-parse", "--show-toplevel"], { cwd: absolutePath, env: cleanEnv });
            const gitRoot = gitRootResult.trim();
            // Only use git remote if this path IS the git root
            if (gitRoot === absolutePath) {
                const { stdout } = await execFileAsync("git", ["remote", "get-url", "origin"], { cwd: absolutePath, env: cleanEnv });
