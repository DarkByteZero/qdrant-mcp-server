...
export class CommitChunker {
     ...
     */
    createChunks(commit, repoPath, diff) {
     ...
     */
    formatChunkContent(commit, commitType, diff) {
        const lines = [];
        // Header section
    ...
    extractDiffPreview(diff) {
        ...
        if (changeLines.length > maxPreviewLines) {
            return (changeLines.slice(0, maxPreviewLines).join("\n") +
     ...
     */
    truncateContent(content, commit, commitType) {
     ...
     */
    createMetadata(commit, commitType, repoPath) {
        return {
