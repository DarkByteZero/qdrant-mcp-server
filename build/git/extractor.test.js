...
describe("GitExtractor", () => {
    ...
    describe("validateRepository", () => {
        it("should return true for valid git repository", async () => {
            mockExecFile.mockResolvedValue({ stdout: ".git", stderr: "" });
            const result = await extractor.validateRepository();
            expect(result).toBe(true);
            expect(mockExecFile).toHaveBeenCalledWith("git", ["rev-parse", "--git-dir"], expect.objectContaining({ cwd: "/test/repo" }));
        });
    ...
    describe("getLatestCommitHash", () => {
        it("should return latest commit hash", async () => {
            ...
            expect(mockExecFile).toHaveBeenCalledWith("git", ["rev-parse", "HEAD"], expect.objectContaining({ cwd: "/test/repo" }));
        });
    });
    describe("getCommitCount", () => {
        it("should return total commit count", async () => {
            ...
            expect(mockExecFile).toHaveBeenCalledWith("git", ["rev-list", "--count", "HEAD"], expect.objectContaining({ cwd: "/test/repo" }));
        });
        it("should return commit count since specific commit", async () => {
            ...
            expect(mockExecFile).toHaveBeenCalledWith("git", ["rev-list", "--count", "abc123..HEAD"], expect.objectContaining({ cwd: "/test/repo" }));
        });
    });
    describe("getCommits", () => {
        ...
        it("should respect maxCommits option", async () => {
            ...
            expect(mockExecFile).toHaveBeenCalledWith("git", expect.arrayContaining(["-n100"]), expect.any(Object));
        });
        it("should add sinceCommit range when specified", async () => {
            mockExecFile.mockResolvedValue({ stdout: "", stderr: "" });
            await extractor.getCommits({ sinceCommit: "abc123" });
            expect(mockExecFile).toHaveBeenCalledWith("git", expect.arrayContaining(["abc123..HEAD"]), expect.any(Object));
        });
        it("should add sinceDate filter when specified", async () => {
            mockExecFile.mockResolvedValue({ stdout: "", stderr: "" });
            await extractor.getCommits({ sinceDate: "2024-01-01" });
            expect(mockExecFile).toHaveBeenCalledWith("git", expect.arrayContaining(["--since=2024-01-01"]), expect.any(Object));
        });
...
+const newLine = true;
 const existingLine = true;`;
            mockExecFile.mockResolvedValue({ stdout: diffOutput, stderr: "" });
            const result = await extractor.getCommitDiff("abc123");
            expect(result).toBe(diffOutput);
            expect(mockExecFile).toHaveBeenCalledWith("git", ["show", "--no-color", "-p", "abc123"], expect.objectContaining({ cwd: "/test/repo" }));
        });
    ...
    describe("getRemoteUrl", () => {
        it("should return remote origin URL", async () => {
            ...
            expect(mockExecFile).toHaveBeenCalledWith("git", ["remote", "get-url", "origin"], expect.objectContaining({ cwd: "/test/repo" }));
        });
...
describe("normalizeRemoteUrl", () => {
    ...
    it("should handle nested paths", () => {
        expect(normalizeRemoteUrl("https://github.com/org/group/subgroup/repo.git")).toBe("org/group/subgroup/repo");
    });
});
