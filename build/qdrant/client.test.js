...
describe("QdrantManager", () => {
    ...
    describe("hybridSearch", () => {
        ...
        it("should use custom limit with appropriate prefetch limit", async () => {
            ...
            expect(mockClient.query).toHaveBeenCalledWith("test-collection", expect.objectContaining({
        ...
        it("should throw error with error.data.status.error message", async () => {
            ...
            await expect(manager.hybridSearch("test-collection", denseVector, sparseVector)).rejects.toThrow('Hybrid search failed on collection "test-collection": Named vector not found');
        });
        it("should throw error with error.message fallback", async () => {
            const denseVector = [0.1, 0.2, 0.3];
            const sparseVector = { indices: [1], values: [0.5] };
            mockClient.query.mockRejectedValue(new Error("Network timeout"));
            await expect(manager.hybridSearch("test-collection", denseVector, sparseVector)).rejects.toThrow('Hybrid search failed on collection "test-collection": Network timeout');
        });
        it("should throw error with String(error) fallback", async () => {
            const denseVector = [0.1, 0.2, 0.3];
            const sparseVector = { indices: [1], values: [0.5] };
            mockClient.query.mockRejectedValue("Unknown error");
            await expect(manager.hybridSearch("test-collection", denseVector, sparseVector)).rejects.toThrow('Hybrid search failed on collection "test-collection": Unknown error');
        });
    });
    describe("addPointsWithSparse", () => {
        ...
        it("should throw error with error.data.status.error message", async () => {
            ...
            await expect(manager.addPointsWithSparse("test-collection", points)).rejects.toThrow('Failed to add points with sparse vectors to collection "test-collection": Sparse vector not configured');
        });
        it("should throw error with error.message fallback", async () => {
            const points = [
                {
            ...
            await expect(manager.addPointsWithSparse("test-collection", points)).rejects.toThrow('Failed to add points with sparse vectors to collection "test-collection": Connection refused');
        });
        it("should throw error with String(error) fallback", async () => {
            const points = [
                {
            ...
            await expect(manager.addPointsWithSparse("test-collection", points)).rejects.toThrow('Failed to add points with sparse vectors to collection "test-collection": Unexpected error');
        });
    });
});
