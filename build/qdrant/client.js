...
export class QdrantManager {
    log = logger.child({ component: "qdrant" });
    client;
    constructor(url = "http://localhost:6333", apiKey) {
    ...
    async createCollection(name, vectorSize, distance = "Cosine", enableSparse = false) {
    ...
    async getCollectionInfo(name) {
        ...
        if (typeof vectorConfig === "object" && vectorConfig !== null) {
            ...
            if ("size" in vectorConfig) {
                ...
                const denseConfig = vectorConfig.dense;
                size = typeof denseConfig.size === "number" ? denseConfig.size : 0;
                distance = denseConfig.distance;
            }
        }
        return {
            name,
    ...
    async addPoints(collectionName, points) {
    ...
    async search(collectionName, vector, limit = 5, filter) {
     ...
     */
    async hybridSearch(collectionName, denseVector, sparseVector, limit = 5, filter, _semanticWeight = 0.7) {
     ...
     */
    async addPointsWithSparse(collectionName, points) {
