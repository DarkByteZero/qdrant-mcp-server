...
export class EmbeddingProviderFactory {
    static create(config) {
        ...
        switch (provider) {
            ...
            case "voyage":
                ...
                return new VoyageEmbeddings(apiKey, model || "voyage-2", dimensions, rateLimitConfig, baseUrl || "https://api.voyageai.com/v1");
            case "ollama":
                return new OllamaEmbeddings(model || "nomic-embed-text", dimensions, rateLimitConfig, baseUrl || "http://localhost:11434");
            default:
    ...
    static createFromEnv() {
        ...
        if (dimensions !== undefined && (isNaN(dimensions) || dimensions <= 0)) {
            throw new Error(`Invalid EMBEDDING_DIMENSIONS: must be a positive integer, got "${process.env.EMBEDDING_DIMENSIONS}"`);
        }
        ...
        if (maxRequestsPerMinute !== undefined &&
            ...
            throw new Error(`Invalid EMBEDDING_MAX_REQUESTS_PER_MINUTE: must be a positive integer, got "${process.env.EMBEDDING_MAX_REQUESTS_PER_MINUTE}"`);
        }
        ...
        if (retryAttempts !== undefined &&
            ...
            throw new Error(`Invalid EMBEDDING_RETRY_ATTEMPTS: must be a non-negative integer, got "${process.env.EMBEDDING_RETRY_ATTEMPTS}"`);
        }
        ...
        if (retryDelayMs !== undefined &&
            ...
            throw new Error(`Invalid EMBEDDING_RETRY_DELAY: must be a non-negative integer, got "${process.env.EMBEDDING_RETRY_DELAY}"`);
        }
